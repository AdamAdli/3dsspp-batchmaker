...
public class BatchExporter {
    ...
    public static void saveBatch(String filepath, Trial[] trials, String analystName, String companyName) throws Exception {
        ...
        for (int i = 0; i < trials.length; i++) {
            //DES 0 "Loads A" "Suzy Anybody" "Hand loads case A" "Company" #
            writer.println("DES 0 \"Row " + Integer.toString(i) +"\" \""+analystName+"\" \"\" \""+companyName+"\" #");
            ...
            writer.println("#"); // No more new lines!
        }
        writer.flush();
        writer.close();
    }

    public static Runnable saveToBatchFile(String filepath, Trial[] trials, String analystName, String companyName, BatchSavedCallback savedCallback, FailCallback failCallback) {
        return () -> {
            try {
                saveBatch(filepath, trials, analystName, companyName);
                savedCallback.onSaved(filepath);
            } catch (Exception ex) {
                failCallback.onBatchFailed(ex.getMessage());
            }
        };
    }

    public interface FailCallback {
        void onBatchFailed(String message);
    }
